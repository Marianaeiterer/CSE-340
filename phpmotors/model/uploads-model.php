...
function allThumbnails($invId){
    $db = phpmotorsConnect();
    $sql = 'SELECT inventory.invId, invMake, invModel, invPrice, invStock, invColor, images.imgPath AS invThumbnail 
    FROM inventory
    INNER JOIN images
    ON images.invId = inventory.invId
    WHERE (images.imgName LIKE "%-tn%") AND (inventory.invId = :invId)';
    $stmt = $db->prepare($sql);
